        ...
        <Container>
          <div className="flex flex-col md:flex-row h-full gap-40">
            <div className="text-white max-w-md md:sticky md:top-20 self-start flex-shrink-0 z-10">
              <h2 className="text-4xl md:text-5xl whitespace-nowrap leading-[4rem] tracking-wider font-semibold text-center md:text-left">
                Get Started In Just<br /> <span className="text-[#0171DF] whitespace-nowrap">3 Simple Steps</span>
              </h2>
              <p className="text-[#FFFFF] text-[20px] mt-4 text-center md:text-left">
            ...
            <div className="flex flex-col space-y-6 flex-grow md:p-0 p-2">
              {steps.map((step, index) => (
                <div
                  key={index}
                  className="flex flex-col items-center p-6 space-y-[20px] bg-[#D2E7FA] rounded-3xl shadow-lg md:w-[600px] md:h-[480px] w-full"
                >
                  <Badge className="self-start bg-[#0171DF] text-white font-light px-4 py-1 rounded-full" variant="outline">
                    Step {index + 1}
                  </Badge>
                  <h2 className="text-[#002147] font-semibold mt-2 text-[32px] md:text-start md:self-start">{step.title}</h2>
                  <p className="text-[#334D6C] text-[20px] font-light ">{step.description}</p>
